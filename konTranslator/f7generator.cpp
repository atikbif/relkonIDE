...
F7Generator::F7Generator(const Display &d): CHGenerator(d)
{

}

QStringList F7Generator::getFileNames()
{
...
int F7Generator::createFiles(TextBlock &varBlock, TextBlock &initBlock, QVector<KonProcess> &prBlock)
{
...
void F7Generator::translateKonToC()
{
...
void F7Generator::createFcuC()
{
    ...
    for(const QString &s: (const QStringList)reqBody) {
        out << s+"\n";
    }
    ...
    out << "unsigned int r1=0,r5=0,r10=0,r100=0,r1000=0;\n";
...
void F7Generator::createIoDefinesH()
{
    QDir dir(PathStorage::getSrcDir());
    if(!dir.exists()) {dir.mkdir(".");}
...
void F7Generator::translateIO(QString &str, int konNum)
{
    bool errExp = false;
...
void F7Generator::translateSitJump(QString &str, int konNum)
{
    QRegExp repExp("\\s*#/R;");
    int pos = 0;
...
void F7Generator::translateStartStopProc(QString &str, int konNum)
{
...
void F7Generator::translateTextBlock(QStringList &strList, QVector<int> &konStrNum)
{
...
void F7Generator::printProcess(QTextStream &out, int procId, int period)
{
