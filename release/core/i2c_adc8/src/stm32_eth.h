...
typedef struct {
  ...
  uint32_t             ETH_JumboFrame;                /* Enable/disable Jumbo frame */
...
#ifdef STM32F20X
/* ETHERNET DMA PTP Desciptors data structure definition  */
typedef struct  {
  volatile u32   Status;                /* Status */
  u32   ControlBufferSize;     /* Control and Buffer1, Buffer2 lengths */
  u32   Buffer1Addr;           /* Buffer1 address pointer */
  u32   Buffer2NextDescAddr;   /* Buffer2 or next descriptor address pointer */
  u32   ExtendedStatus;        /* Extended status for PTP receive descriptor */
  u32   Reserved1;             /* Reserved */
  u32   TimeStampLow;          /* Time Stamp Low value for transmit and receive */
  u32   TimeStampHigh;         /* Time Stamp High value for transmit and receive */
} ETH_DMAPTPDESCTypeDef;
#endif
...
/* MAX_ETH_PAYLOAD is in fact the size of Big Packet. For instance DHCP request is 592bytes long!
Take care to put the right size depending on your network. 1500 is for Ethernet max frame.
It is MSS = Max segment size + TCP header or UDP header + IP header
*/
//#define MAX_ETH_PAYLOAD        600    /* Maximum Ethernet payload size */
#define JUMBO_FRAME_PAYLOAD    9000    /* Jumbo frame payload size */
#define ETH_MAX_PACKET_SIZE    ((ETH_HEADER) + (ETH_EXTRA) + (MAX_ETH_PAYLOAD) + (ETH_CRC))
...
#define ETH_DMARxDesc_IPV4HCE     ((uint32_t)0x00000080)  /* IPC Checksum Error/Giant Frame: Rx Ipv4 header checksum error   */
#define ETH_DMARxDesc_RxLongFrame ((uint32_t)0x00000080)  /* (Giant Frame)Rx - frame is longer than 1518/1522   */
...
#define ETH_DMAPTPRxDesc_PTPSA    ((uint32_t)0x00004000)  /* PTP Snapshots Available */
#define ETH_DMAPTPRxDesc_PTPFT    ((uint32_t)0x00002000)  /* PTP Frame Type */
#define ETH_DMAPTPRxDesc_PTPV     ((uint32_t)0x00001000)  /* PTP Version */
...
#define ETH_JumboFrame_Enable     ((uint32_t)0x00100000)
...
#define IS_ETH_DMAPTPRxDESC_GET_EXTENDED_FLAG(FLAG) (((FLAG) == ETH_DMAPTPRxDesc_PTPSA) || \
                                                     ((FLAG) == ETH_DMAPTPRxDesc_PTPFT) || \
                                                     ((FLAG) == ETH_DMAPTPRxDesc_PTPV) || \
...
#ifdef STM32F20X
#define ETH_RxDMABurstLength_8xPBL_8Beat    ((uint32_t)0x01020000)  /* maximum number of beats to be transferred in one RxDMA transaction is 8 */
#define ETH_RxDMABurstLength_8xPBL_16Beat   ((uint32_t)0x01040000)  /* maximum number of beats to be transferred in one RxDMA transaction is 16 */
#define ETH_RxDMABurstLength_8xPBL_32Beat   ((uint32_t)0x01080000)  /* maximum number of beats to be transferred in one RxDMA transaction is 32 */
#define ETH_RxDMABurstLength_8xPBL_64Beat   ((uint32_t)0x01100000)  /* maximum number of beats to be transferred in one RxDMA transaction is 64 */
#define ETH_RxDMABurstLength_8xPBL_128Beat  ((uint32_t)0x01200000)  /* maximum number of beats to be transferred in one RxDMA transaction is 128 */
#define ETH_RxDMABurstLength_8xPBL_256Beat  ((uint32_t)0x01400000)  /* maximum number of beats to be transferred in one RxDMA transaction is 256 */
#endif
...
#ifdef STM32F20X
#define ETH_TxDMABurstLength_8xPBL_8Beat    ((uint32_t)0x01000100)  /* maximum number of beats to be transferred in one TxDMA (or both) transaction is 8 */
#define ETH_TxDMABurstLength_8xPBL_16Beat   ((uint32_t)0x01000200)  /* maximum number of beats to be transferred in one TxDMA (or both) transaction is 16 */
#define ETH_TxDMABurstLength_8xPBL_32Beat   ((uint32_t)0x01000400)  /* maximum number of beats to be transferred in one TxDMA (or both) transaction is 32 */
#define ETH_TxDMABurstLength_8xPBL_64Beat   ((uint32_t)0x01000800)  /* maximum number of beats to be transferred in one TxDMA (or both) transaction is 64 */
#define ETH_TxDMABurstLength_8xPBL_128Beat  ((uint32_t)0x01001000)  /* maximum number of beats to be transferred in one TxDMA (or both) transaction is 128 */
#define ETH_TxDMABurstLength_8xPBL_256Beat  ((uint32_t)0x01002000)  /* maximum number of beats to be transferred in one TxDMA (or both) transaction is 256 */
#endif
...
#define ETH_PTPPPSCR    ((uint32_t)0x0000072C)  /* PTP PPSCR register */
...
#define ETH_PTP_SnapshotAllFrames              ((uint32_t)0x00000100)  /* Time stamp snapshot for all frames enable */
...
#define IS_ETH_PTP_SNAPSHOT(SNAPSHOT) (((SNAPSHOT) == ETH_PTP_SnapshotMasterMessage) || \
                                       ...
                                       ((SNAPSHOT) == ETH_PTP_SnapshotAllFrames))
#endif
